...
public:
  ...
  void get_eigenmode_coefficients_multi_dp(
      dft_flux flux, const volume &eig_vol, diffractedplanewave *dps, int num_dps, int parity,
      double eig_resolution, double eigensolver_tol, std::complex<double> *coeffs, double *vgrp,
      kpoint_func user_kpoint_func = 0, void *user_kpoint_data = 0, vec *kpoints = 0, vec *kdom = 0,
      double *cscale = 0, direction d = NO_DIRECTION);
